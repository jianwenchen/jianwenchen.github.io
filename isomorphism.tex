...
\end{Thm}
\begin{proof}[证明]
  设$(G,\circ)$为一个群。$\forall a\in G$，令$f_a:G\to G$，$\forall x\in G$，$f_a(x)=ax$，则$f_a$为$G$到$G$的一一对应。
...
令$\phi:G\to L(G)$，$\forall x\in G$，$\phi(a)=f_a$，则$\phi$为双射（$\phi$为单射，这是因为如果$\forall a,b\in G$，如果$\phi(a)=\phi(b)$，则$f_a=f_b$，从而$f_a(e)=f_b(e)$，即$ae=be$，于是$a=b$；$\phi$为满射，这是因为对任意的$f\in L(G)$，$\exists a\in G$使得$f=f_a$，从而$\phi(a)=f_a=f$）。

$\forall a,b\in G, \phi(ab)=f_{ab}=f_a\circ f_b=\phi(a)\circ \phi(b)$，因此$\phi$为从$G$到$L(G)$的一个同构，即$G\equiv L(G)$。
\end{proof}
