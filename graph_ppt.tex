...
\begin{frame}
  \frametitle{6.2 基本定义}
  \begin{Def}
    图$G$称为\alert{$r$度正则图}，如果$G$的每个顶点的度都等于$r$。３度正则图也称为三次图。
...
\begin{proof}[证明]\justifying\let\raggedright\justifying
  \pause用反证法。\pause假设$G$不连通，\pause则$G$至少有两个支。\pause设$G_1=(V_1,E_1)$为其中的一个支，\pause其他各支构成的子图为$G_2=(V_2,E_2)$。\pause取$V_1$中的任意一个顶点$u$和$V_2$中的任意一个顶点$v$，\pause则顶点$u$和顶点$v$不邻接并且
