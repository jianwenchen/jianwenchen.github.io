...
\begin{frame}
    \begin{Exercise}
      证明：唯一没有三角形的$(p,[\frac{p^2}{4}])$图为$K(\lfloor \frac{p}{2} \rfloor,\lceil \frac{p}{2} \rceil )$。
\end{Exercise}
\pause证明：用数学归纳法证明以下结论：唯一没有三角形的包含$p$个顶点且边数$q\geq [\frac{p^2}{4}]$的图一定为$K(\lfloor \frac{p}{2} \rfloor,\lceil \frac{p}{2} \rceil )$。\pause施归纳于顶点数$p$，\pause只证$p$为奇数的情况，$p$为偶数的情况是类似的。
