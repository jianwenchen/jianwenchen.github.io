...
\begin{proof}[证明]
  ...
  以下用数学归纳法证明$n$个元素按照$a_1,a_2,\cdots,a_n$的次序任意加括号所得到的乘积都等于$((((a_1\circ a_2)\circ a_3)\circ \cdots )\circ a_{n-1})\circ a_n$，施归纳于$n$。

  当$n=1,2$时结论显然成立。

  假设当$n<k$时结论成立，往证当$n=k$时结论也成立。

  对$k$个元素按$a_1,a_2,\cdots,a_k$的次序不论用什么方法加括号确定计算方案，最后一步必是两个元素的乘积，不妨设为$b_1\circ b_2$，这里$b_1$为前$i$个元素按照$a_1,a_2,\cdots,a_i$的次序以某种方式加括号之积，
  而$b_2$为后$k-i$个元素按照$a_{i+1},a_{i+2},\cdots,a_k$的次序以某种方式加括号之积，则
